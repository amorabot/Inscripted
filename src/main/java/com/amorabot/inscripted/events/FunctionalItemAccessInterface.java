...
public class FunctionalItemAccessInterface {
    ...
    public static boolean isWeapon(PersistentDataContainer dataContainer){
        ...
        return dataContainer.has(new NamespacedKey(INSCRIPTED_PLUGIN, weaponKey), new PersistentDataType.BooleanPersistentDataType()); //If its a weapon, it has this container
    }
    ...
    public static boolean isEquipableWeapon(PersistentDataContainer dataContainer){
        if (isWeapon(dataContainer)) {
            return Boolean.TRUE.equals(dataContainer.get(new NamespacedKey(INSCRIPTED_PLUGIN, weaponKey), new PersistentDataType.BooleanPersistentDataType()));
        }
        return false;
    }
    ...
    public static void serializeWeapon(Weapon weaponData, PersistentDataContainer dataContainer){
        dataContainer.set(new NamespacedKey(INSCRIPTED_PLUGIN, weaponDataKey), new GenericItemContainerDataType<>(Weapon.class), weaponData);
        dataContainer.set(new NamespacedKey(INSCRIPTED_PLUGIN, weaponKey), new PersistentDataType.BooleanPersistentDataType(), weaponData.isIdentified());
    }
    ...
    public static Weapon deserializeWeapon(PersistentDataContainer dataContainer){ // Optional
        if (isWeapon(dataContainer)){
            Weapon weaponData = dataContainer.get(new NamespacedKey(INSCRIPTED_PLUGIN, weaponDataKey), new GenericItemContainerDataType<>(Weapon.class));
    ...
    private static void identifyWeapon(PersistentDataContainer dataContainer){
        if (isWeapon(dataContainer)){
            dataContainer.set(new NamespacedKey(INSCRIPTED_PLUGIN, weaponKey), new PersistentDataType.BooleanPersistentDataType(), true);
        }
    }
    ...
    public static boolean isArmor(PersistentDataContainer dataContainer){
        if (dataContainer==null){return false;}
        return dataContainer.has(new NamespacedKey(INSCRIPTED_PLUGIN, armorKey), new PersistentDataType.BooleanPersistentDataType());
    }
    public static boolean isEquipableArmor(PersistentDataContainer dataContainer){
        if (isArmor(dataContainer)) {
            return Boolean.TRUE.equals(dataContainer.get(new NamespacedKey(INSCRIPTED_PLUGIN, armorKey), new PersistentDataType.BooleanPersistentDataType()));
        }
        return false;
    }
    ...
    public static void serializeArmor(Armor armorData, PersistentDataContainer dataContainer){
        dataContainer.set(new NamespacedKey(INSCRIPTED_PLUGIN, armorDataKey), new GenericItemContainerDataType<>(Armor.class), armorData);
        dataContainer.set(new NamespacedKey(INSCRIPTED_PLUGIN, armorKey), new PersistentDataType.BooleanPersistentDataType(), armorData.isIdentified());
    }
    public static Armor deserializeArmor(PersistentDataContainer dataContainer){
        if (isArmor(dataContainer)){
            Armor armorData = dataContainer.get(new NamespacedKey(INSCRIPTED_PLUGIN, armorDataKey), new GenericItemContainerDataType<>(Armor.class));
    ...
    private static void identifyArmor(PersistentDataContainer dataContainer){
        if (isWeapon(dataContainer)){
            dataContainer.set(new NamespacedKey(INSCRIPTED_PLUGIN, armorKey), new PersistentDataType.BooleanPersistentDataType(), true);
        }
    }
