...
public class GlobalCooldownManager {
    ...
    public static boolean skillcastBy(UUID playerID, PlayerAbilities abilityUsed){
        ...
        if (!abilitiesGDC.containsKey(type)){
            ...
            playerCDMap.put(playerID, new GlobalCooldown(abilityUsed.getCooldownInSeconds()*1000, castTime));
        ...
        if (!abilitiesGDC.get(type).containsKey(playerID)){
            ...
            abilitiesGDC.get(type).put(playerID, new GlobalCooldown(abilityUsed.getCooldownInSeconds()*1000, castTime));
            return true;
        }
        //A GCD object is accessible
        GlobalCooldown playerGCD = abilitiesGDC.get(type).get(playerID);
        if (playerGCD.canBeCast()){
            playerGCD.setBaseGCD(abilityUsed.getCooldownInSeconds()*1000);
            playerGCD.setLastCastTime(castTime);
            //TODO:might need to put playerGCD back
            return true;
        }
        return false;
    }
